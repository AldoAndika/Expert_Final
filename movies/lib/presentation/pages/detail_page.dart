...
import '../bloc/movie_recommendations/bloc.dart';

class DetailPageMovie extends StatefulWidget {
  static const routeName = '/movie-detail';
  final int id;
  const DetailPageMovie({Key? key, required this.id}) : super(key: key);

  @override
  _MovieDetailPageState createState() => _MovieDetailPageState();
}

class _MovieDetailPageState extends State<DetailPageMovie> {
  @override
  void initState() {
    super.initState();
    Future.microtask(() {
      context.read<DetailBlocMovie>().add(OnMovieDetailCalled(widget.id));
      context
          .read<MovieRecommendationsBloc>()
          .add(OnMovieRecommendationsCalled(widget.id));
      context.read<WatchlistBlocMovies>().add(FetchWatchlistStatus(widget.id));
    });
  }
  ...
  Widget build(BuildContext context) {
    final isMovieAddedToWatchlist =
        context.select<WatchlistBlocMovies, bool>((bloc) {
    ...
    return Scaffold(
      ...
      body: BlocBuilder<DetailBlocMovie, DetailState>(
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                if (!widget.isAddedWatchlist) {
                                  context
                                      .read<WatchlistBlocMovies>()
                                      .add(AddMovieToWatchlist(widget.movie));
                                } else {
                                  context.read<WatchlistBlocMovies>().add(
                                ...
                                final state =
                                    BlocProvider.of<WatchlistBlocMovies>(
                                            context)
                                        .state;
